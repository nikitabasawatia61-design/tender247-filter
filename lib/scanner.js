...
async function scanClosingDate(isoDate, opts) {
  ...
  onProgress({
    phase: 'candidates',
    message: `${candidates.length} matched · ${supplyRelated.length} supply · after field + supply + API rules`,
  ...
  supplyRelated.sort((a, b) => (a.emd || 999999) - (b.emd || 999999));
  ...
  const result = {
    ...
    rules: {
      ...
      supply: 'Purchase/supply/procurement of goods → Supply tab (not matched)',
    ...
    summary: {
      ...
      supply_related: supplyRelated.length,
