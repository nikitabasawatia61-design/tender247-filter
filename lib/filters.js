...
/** IT system/solution procurement — not goods supply (stay in Matched) */
function isItSystemProcurement(t) {
  if (/\b(eoi|rfp|rfq|request for proposal)\b/.test(t) && /\b(system|software|application|platform|solution|module|portal|suite)\b/.test(t)) {
    return true;
  }
  if (/\bprocurement of\b.*\b(system|software|application|platform|solution|module|portal|erp|website|web portal|ai based|saas)\b/.test(t)) {
    return true;
  }
  if (/\btender evaluation system\b/.test(t)) return true;
  if (/\bevaluation system\b/.test(t)) return true;
  return false;
}
...
function isSupplyRelated(item) {
  ...
  if (isItSystemProcurement(t)) return false;
